#!/usr/bin/env python3
"""Build and measure ML-KEM allocation and bulk-copy budgets."""
...
EXPECTED_BUDGETS = {
    "mlkem768-keygen": {"allocationCalls": 17, "allocationBytes": 20_640},
    "mlkem768-encap": {"allocationCalls": 5, "allocationBytes": 6_096},
    "mlkem768-decap": {"allocationCalls": 11, "allocationBytes": 9_808},
    "mlkem1024-keygen": {"allocationCalls": 17, "allocationBytes": 31_008},
    "mlkem1024-encap": {"allocationCalls": 5, "allocationBytes": 7_632},
    "mlkem1024-decap": {"allocationCalls": 11, "allocationBytes": 12_336},
}
...
def validate_budget(workload: str, derived: dict[str, Any]) -> list[str]:
    ...
    failures: list[str] = []
    for name in ("allocationCalls", "allocationBytes"):
        if values[name] != expected[name]:
            failures.append(f"{name} expected {expected[name]}, observed {values[name]}")
    if values["bulkCopyBytes"] != 0:
        failures.append(f"bulkCopyBytes expected 0, observed {values['bulkCopyBytes']}")
    if values["callocCalls"] != 0 or values["reallocCalls"] != 0:
        failures.append("calloc/realloc must not occur per operation")
    if workload.endswith(("-encap", "-decap")) and values["mallocCalls"] != 0:
        failures.append("in-place encapsulation/decapsulation performed general malloc")
...
def main() -> int:
    ...
    build_parent = ROOT / ".build" / "benchmark-mlkem-memory"
    ...
    with tempfile.TemporaryDirectory(prefix="run-", dir=build_parent) as temporary:
        ...
        worker = scratch / "arm64-apple-macosx" / "release" / "swift-ssl-mlkem-benchmark"
        if not worker.is_file():
            raise MeasurementError(f"missing benchmark worker: {worker}")

        probe = run_root / "libSwiftSSLMLKEMAllocationProbe.dylib"
        contract = run_root / "allocation-probe-contract"
        probe_source = snapshot / "Benchmarks/MLKEM/AllocationProbe/MLKEMAllocationProbe.c"
        contract_source = snapshot / "Benchmarks/MLKEM/AllocationProbe/AllocationProbeContract.c"
        common_compile = [
            str(clang),
            "-O2",
            "-std=c11",
        ...
        for parameter_set in ("768", "1024"):
            for operation in ("keygen", "encap", "decap"):
                workload = f"mlkem{parameter_set}-{operation}"
                ...
                results[workload] = {
                    "parameterSet": int(parameter_set),
                    "operation": operation,
                    "passed": passed,
                    "failures": failures,
        ...
        artifact = {
            ...
            "scope": (
                "public entropy-injected key generation and public in-place "
                "encapsulation/decapsulation loops; setup excluded"
            ),
            "limitations": [
        ...
        if output is None:
            ...
            output = ROOT / "Benchmarks/MLKEM/Results" / f"{timestamp}-native-mlkem-memory.json"
