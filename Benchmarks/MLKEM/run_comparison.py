#!/usr/bin/env python3
"""Build and compare Pure Swift and BoringSSL ML-KEM implementations."""
...
WORKLOADS = tuple(
    ...
    for parameter_set in (768, 1024)
    for operation in ("keygen", "encap", "decap")
)
INITIAL_ITERATIONS = {
    (768, "keygen"): 30_000,
    (768, "encap"): 50_000,
...
FIXTURE_PATTERN = re.compile(
    r"^FIXTURE,([0-9a-f]+),([0-9a-f]+),([0-9a-f]{64})$"
)
SECRET_PATTERN = re.compile(r"^SECRET,([0-9a-f]{64})$")
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build and compare Pure Swift and pinned BoringSSL ML-KEM workers "
    ...
    parser.add_argument(
        "--output",
        type=Path,
        help="Result JSON path; defaults to Benchmarks/MLKEM/Results.",
    )
    parser.add_argument(
        "--samples",
...
def inspect_worker_codegen(
    ...
    required_swift_symbols = (
        "MLKEMArithmeticO10forwardNTT",
        "MLKEMArithmeticO10inverseNTT",
        "MLKEMArithmeticO12multiplyNTTs",
        "KeccakX2Core",
        "MLKEM768O11encapsulate",
        "MLKEM1024O11decapsulate",
    )
    missing_swift = [value for value in required_swift_symbols if value not in swift_symbols]
    if missing_swift:
        raise BenchmarkError(f"Swift ML-KEM symbols are missing: {missing_swift}")
    forward_blocks = code_blocks_containing(swift_disassembly, "MLKEMArithmeticO10forwardNTT")
    multiply_blocks = code_blocks_containing(swift_disassembly, "MLKEMArithmeticO12multiplyNTTs")
    forward_umin = sum(block.count("umin.8h") for block in forward_blocks)
    multiply_umin = sum(block.count("umin.8h") for block in multiply_blocks)
    sha3_counts = {
        instruction: swift_disassembly.count(instruction)
        for instruction in ("eor3.16b", "rax1.2d", "xar.2d", "bcax.16b")
    }
    if forward_umin < 80 or multiply_umin < 18:
        raise BenchmarkError(
            "Swift ML-KEM NTT code generation lost the branch-free SIMD reduction shape"
        )
    if any(count == 0 for count in sha3_counts.values()):
        raise BenchmarkError("Swift ML-KEM code generation lost ARM SHA3 instructions")
    ...
    if missing_boringssl:
        raise BenchmarkError(f"BoringSSL ML-KEM symbols are missing: {missing_boringssl}")
    ...
    return {
        "swift": {
            ...
            "forwardNTTBranchFreeSIMDReductionCount": forward_umin,
            "multiplyNTTBranchFreeSIMDReductionCount": multiply_umin,
...
def build_workers(
    *,
    ...
    driver_source = swift_source / "Benchmarks/MLKEM/BoringSSLDriver"
    configure_command = [
        cmake,
        "-S",
        str(driver_source),
    ...
    for module in ("SwiftSSLCore", "SwiftSSLCrypto", "SwiftSSL", "SwiftSSLMLKEMBenchmark"):
        module_lines = [
            line
    ...
    required_boringssl_sources = {
        (driver_source / "MLKEMBenchmark.cpp").resolve(),
        (boringssl_source / "crypto/mlkem/mlkem.cc").resolve(),
...
def parse_fixture_record(record: str, parameter_set: int) -> tuple[str, str, str]:
    match = FIXTURE_PATTERN.fullmatch(record)
    if match is None:
        raise BenchmarkError("worker emitted a malformed interoperability fixture")
    expected_public_bytes = 1_184 if parameter_set == 768 else 1_568
    expected_ciphertext_bytes = 1_088 if parameter_set == 768 else 1_568
    if len(match.group(1)) != expected_public_bytes * 2:
        raise BenchmarkError("interoperability public key has an invalid length")
    if len(match.group(2)) != expected_ciphertext_bytes * 2:
        raise BenchmarkError("interoperability ciphertext has an invalid length")
...
def parse_secret_record(record: str) -> str:
    ...
    if match is None:
        raise BenchmarkError("worker emitted a malformed decapsulation record")
...
def validate_interoperability(
    ...
    fixture_inputs = {
        768: {
            ...
            "message": "de00df8311d05e830c71b568da4cf0f0c7f458d94139de92d3c5f21c3ca48ad8",
        },
        1024: {
            "seed": (
            ...
            "message": "8fb8906c16c4d8190f59c254b1eaa537d46a71e864f2abac5cc87b79420ca762",
        },
    }
    evidence: dict[str, Any] = {}
    for parameter_set, inputs in fixture_inputs.items():
        parameter = str(parameter_set)
        swift_record = invoke_validation(
            swift_worker,
            ["--fixture", parameter, inputs["seed"], inputs["message"]],
        )
        swift_public, swift_ciphertext, swift_secret = parse_fixture_record(
            swift_record, parameter_set
        )
        boring_validation = invoke_validation(
            boringssl_worker,
            [
                "--validate",
                parameter,
        ...
        if boring_validation != "VALIDATED":
            raise BenchmarkError("BoringSSL did not validate the Swift fixture")

        mutated = bytearray.fromhex(swift_ciphertext)
        mutated[len(mutated) // 2] ^= 0x80
        mutated_hex = mutated.hex()
        swift_rejection = parse_secret_record(
            invoke_validation(
                swift_worker,
                ["--decap", parameter, inputs["seed"], mutated_hex],
            )
        )
        boring_rejection = parse_secret_record(
            invoke_validation(
                boringssl_worker,
                ["--decap", parameter, inputs["seed"], mutated_hex],
            )
        )
        if swift_rejection != boring_rejection or swift_rejection == swift_secret:
            raise BenchmarkError("ML-KEM implicit rejection differs between implementations")

        boring_record = invoke_validation(
            boringssl_worker,
            ["--fixture", parameter, inputs["seed"]],
        )
        boring_public, boring_ciphertext, boring_secret = parse_fixture_record(
            boring_record, parameter_set
        )
        swift_validation = invoke_validation(
            swift_worker,
            [
                "--validate",
                parameter,
        ...
        if swift_validation != "VALIDATED":
            raise BenchmarkError("SwiftSSL did not validate the BoringSSL fixture")
        evidence[f"mlkem{parameter_set}"] = {
            "seed": inputs["seed"],
            ...
            "implicitRejectionSecretSHA256": hashlib.sha256(
                bytes.fromhex(swift_rejection)
            ).hexdigest(),
            "swiftToBoringSSL": "validated",
            "boringSSLToSwift": "validated",
            "implicitRejection": "identical",
            "passed": True,
        }
    return {
        "method": "deterministic seeded keys, bidirectional ciphertext validation, and mutated-ciphertext implicit rejection",
        "parameterSets": evidence,
        "passed": True,
    }
...
def converge(
    ...
    for index in range(MAXIMUM_CONVERGENCE_ROUNDS):
        ...
        if convergence_reached(swift_values) and convergence_reached(boringssl_values):
            return rounds
    raise BenchmarkError(
        f"timing did not converge for ML-KEM-{parameter_set} {operation}"
    )
...
def calibrate(
    ...
    raise BenchmarkError(
        ...
        f"ML-KEM-{parameter_set} {operation}"
    )
...
def main() -> int:
    ...
    if output is None:
        output = (
            SCRIPT_DIRECTORY
            / "Results"
            / f"{utc_file_timestamp()}-native-mlkem.json"
        )
    ...
    if build_root is None:
        parent = REPOSITORY_ROOT / ".build" / "benchmark-mlkem"
    ...
    for parameter_set, operation in workload_order:
        ...
        key = f"mlkem{parameter_set}-{operation}"
        results[key] = {
    ...
    artifact = {
        ...
        "memoryEvidence": {
            "publicEncapsulationOutput": "caller-owned MutableSpan",
            "publicDecapsulationOutput": "caller-owned MutableSpan",
        ...
        "workloadOrder": [f"mlkem{p}-{o}" for p, o in workload_order],
        "results": results,
        "completedAt": utc_now(),
    }
