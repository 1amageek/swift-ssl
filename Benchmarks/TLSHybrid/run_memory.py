#!/usr/bin/env python3
"""Build and measure X25519MLKEM768 TLS allocation and bulk-copy budgets."""
...
EXPECTED_BUDGETS = {
    "client-offer": {"allocationCalls": 17, "allocationBytes": 15_656},
    "server-accept": {"allocationCalls": 15, "allocationBytes": 14_464},
    "roundtrip": {"allocationCalls": 45, "allocationBytes": 40_032},
    "x25519-public": {"allocationCalls": 0, "allocationBytes": 0},
    "x25519-shared": {"allocationCalls": 0, "allocationBytes": 0},
}
...
def main() -> int:
    ...
    build_parent = ROOT / ".build/benchmark-tls-hybrid-memory"
    ...
    with tempfile.TemporaryDirectory(prefix="run-", dir=build_parent) as temporary:
        ...
        worker = (
            scratch
            / "arm64-apple-macosx/release/swift-ssl-tls-hybrid-benchmark"
        )
        if not worker.is_file():
            raise MeasurementError(f"missing benchmark worker: {worker}")

        probe = run_root / "libSwiftSSLTLSHybridAllocationProbe.dylib"
        ...
        support.run(common_compile + ["-dynamiclib", str(probe_source), "-o", str(probe)])
            ...
            for iterations in ITERATION_COUNTS:
                for repetition in range(REPETITIONS):
                    measured = support.run(
                        [str(worker), "--memory", workload, str(iterations)],
                        env=injected_env,
                    )
        ...
        if output is None:
            ...
            output = (
                ROOT
                / "Benchmarks/TLSHybrid/Results"
                / f"{timestamp}-native-tls-hybrid-memory.json"
            )
