...
The profile owns TLS 1.3 records, transcript/key schedule, main- and post-handshake client authentication, encrypted NewSessionTicket transport, ticket/state/PSK binder values, 0-RTT record delivery/discard, post-handshake KeyUpdate, close, and supported modern extensions. It consumes and emits byte streams but performs no I/O. Application traffic destinations are caller-owned unpublished storage: segmented AEAD writes outbound ciphertext directly into the final record owner, and authenticated inbound plaintext is returned in the final caller owner without an intermediate payload allocation. Decrypted control plaintext is consumed through a scoped borrow and wiped before return. Application traffic and exporter secrets use the transcript through Server Finished; a distinct resumption-master owner is created only from the transcript through Client Finished. Ticket issuance returns both the encrypted wire output and the move-only server state that the application must persist. That state carries whether the issuing connection authenticated the peer's certificate proof; a state without this provenance cannot satisfy a later required-client-authentication policy. Server ClientHello handling evaluates PSK identity, age, binder, cipher, ALPN, and authentication policy exactly once. The move-only state is consumed only after the offered identity and age select it; if external credential selection is needed, suspension occurs after that single decision and does not re-enter PSK evaluation. Early-data replay authorization and the resumed handshake remain explicit engine operations. Cross-process replay coordination and ticket persistence remain application policy boundaries.
