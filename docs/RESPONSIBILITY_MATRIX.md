...
| Classical KEX | X25519, P-256, P-384, P-521 | Required | X25519 and P-256 ECDH are implemented in `SwiftSSLCrypto` and `SwiftSSL`; P-384/P-521 remain required for a complete key profile |
| Classical signature | Ed25519; ECDSA P-256/384/521; RSA-PSS SHA-256/384/512 | Required | Ed25519 and P-256 ECDSA verification are callable for the X.509 path; TLS authentication, P-384/P-521, signing, and RSA-PSS remain required |
...
| QUIC RFC 9001 | Required | TLS handshake bytes and secret events only; `QUICInitialSecrets` implements RFC 9001 v1 Initial HKDF derivation |
...
| P-256 ECDH | Fixed eight-limb field/scalar arithmetic, Jacobian scalar multiplication, SEC1 uncompressed point validation, noncopyable private/shared-secret owners, and scoped public-key borrows | Independent field multiplication/inversion fixtures, deterministic generator and ECDH vectors, invalid scalar/point failures, Native façade, WASI, Embedded WASI, and ASan | Constant-time audit, independent differential corpus, measured allocation/copy counts, formal benchmark, and security review |
...
| Remaining cryptography | P-384/P-521, ECDSA signing and P-384/P-521 verification, RSA-PSS, ML-KEM, ML-DSA, HPKE, and hybrid constructions remain outside the callable implementation | Native compilation of protocols only | Concrete implementations, official vectors, negative/differential/target/performance gates |
...
| X.509 | Certificate/TBSCertificate/Validity/signature/SPKI structural parser, Ed25519 and P-256 ECDSA-with-SHA2 signature verification, strict v3 extension parsing, SPKI, and unencrypted PKCS #8 owners | Native X509 tests for certificate ranges/time fields, Ed25519 and P-256 ECDSA SHA-256/384/512 signatures, modified-signature failures, duplicate extensions, X25519 SPKI, and PKCS #8 version failures | Path building, identity, revocation, interoperability, and constant-time/differential release gates |
| TLS/DTLS | TLS 1.3 HKDF key schedule, AEAD record protector, synchronous X25519/Ed25519 stream handshake engine for all three TLS 1.3 AEAD suites, encrypted NewSessionTicket transport, PSK extension/binder primitives, server ticket-age validation, single-use PSK resumption, post-handshake KeyUpdate, SAN-only identity helper, and bounded DTLS replay window | Native full and resumed handshake completion, certificate-window rejection, ticket age/binder rejection, encrypted ticket round trip and resumption-state derivation, PSK extension round trip, KeyUpdate request/response and post-update data, all suite round trips, resumption known answer, SAN identity tests, replay-window boundaries, and Native/WASI/Embedded WASI target validation | 0-RTT, external credential/trust capabilities, DTLS flights/ACK/timers/epochs, full state/interop target evidence |
| QUIC TLS integration | Profile actions, checked batch, noncopyable traffic-secret event, ordered action/secret step output, and RFC 9001 v1 Initial secret/key/IV/header-protection derivation | Focused Native Initial-secret vector and target-validation execution plus existing action/secret tests | TLS engine integration, capability correlation, packet protection, interoperability |
