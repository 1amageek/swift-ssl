...
| ML-KEM-768/1024 | Callable FIPS 203 KEM and in-place KEM implementations with noncopyable private/shared-secret owners, immutable expanded keys, implicit rejection, SIMD NTT, and two-stream Keccak | NIST ACVP/TR1 fixtures, arithmetic differential/boundary tests, typed no-write failures, focused ASan, six optimized secure-wipe code-generation configurations, Native/WASI/Embedded WASI façade execution, pinned-BoringSSL bidirectional interoperability, and a committed formal 30-pair benchmark whose six workloads pass the `1.10x` lower-confidence-bound gate | Measured allocation/copy counts, broader external corpus, automated constant-time review, and security review |
| Remaining cryptography | ML-DSA and approved hybrid constructions remain outside the callable implementation; ML-KEM and X25519 HPKE are callable; RSA-PSS and NIST ECDSA are verification-only | Native vectors cover FIPS 203 and RFC 9180 X25519 HPKE; RSA-PSS has Montgomery differential, sanitizer, and three-target runtime evidence; NIST ECDSA has independent verification vectors | TLS hybrid integration, ML-DSA, ECH, approved hybrid constructions, and their completion gates |
...
| TLS/DTLS | One record-independent TLS 1.3 semantic core shared by Stream and QUIC, narrow client/server protocols, HKDF key schedule, AEAD Stream record adapter, synchronous X25519/Ed25519 authentication for all three TLS 1.3 AEAD suites, encrypted NewSessionTicket transport, PSK extension/binder primitives, server ticket-age validation, single-use PSK resumption, post-handshake KeyUpdate, SAN-only identity helper, and bounded DTLS replay window | Native full and resumed Stream/Core/QUIC handshake completion, Ed25519 CertificateVerify with X.509 fixtures, certificate-window rejection, ticket age/binder rejection, encrypted ticket round trip and resumption-state derivation, KeyUpdate request/response and post-update data, all suite round trips, focused Stream AddressSanitizer execution, replay-window boundaries, and dedicated Native/WASI/Embedded WASI QUIC/core runtime validation | RFC 8448 corpus, 0-RTT, external credential/trust capabilities, DTLS flights/ACK/timers/epochs, allocation/copy measurement, and external interoperability |
...
| `SwiftSSL` façade | Explicit AES-GCM, SHA-2, SHA-3/SHAKE, HMAC-SHA-256, HKDF-SHA-256, X25519, Ed25519, ML-KEM-768/1024, and verification-only P-256 ECDSA adapters; lower modules remain separately importable products | Symmetric, hash/MAC/KDF, X25519, Ed25519, ML-KEM success/failure, and P-256 verification have Native/WASI/Embedded WASI execution | Full application-facing TLS composition and a reviewed stability surface |
