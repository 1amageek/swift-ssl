...
| Remaining cryptography | P-384/P-521 ECDSA signing, ML-KEM, ML-DSA, HPKE, and hybrid constructions remain outside the callable implementation; P-256 signing, RSA-PSS, and P-384/P-521 verification are validation-only | Native vectors now cover RFC 6979 P-256 signing, RSA-PSS, and P-384/P-521 verification; protocol compilation remains separate | Constant-time/differential release gates for P-256/P-384/P-521/RSA-PSS, TLS selection, signing, ML-KEM, ML-DSA, HPKE, and hybrid constructions |
...
| TLS/DTLS | TLS 1.3 HKDF key schedule, AEAD record protector, synchronous X25519/Ed25519 stream handshake engine for all three TLS 1.3 AEAD suites, validation-only P-256 CertificateVerify codec/signing path, encrypted NewSessionTicket transport, PSK extension/binder primitives, server ticket-age validation, single-use PSK resumption, post-handshake KeyUpdate, SAN-only identity helper, and bounded DTLS replay window | Native full and resumed handshake completion, certificate-window rejection, ticket age/binder rejection, encrypted ticket round trip and resumption-state derivation, PSK extension round trip, KeyUpdate request/response and post-update data, all suite round trips, resumption known answer, SAN identity tests, replay-window boundaries, and Native/WASI/Embedded WASI target validation | P-256 TLS selection until constant-time/differential gates, 0-RTT, external credential/trust capabilities, DTLS flights/ACK/timers/epochs, full state/interop target evidence |
