...
| HPKE | RFC 9180 X25519 DHKEM Base, PSK, Auth, and AuthPSK modes with HKDF-SHA256/384/512 and AES-GCM/ChaCha20-Poly1305 | Required | X25519 profile is callable with distinct noncopyable sender/recipient contexts, stack-scoped DH output, one prepared AEAD schedule per context, reused SHA-256 HMAC schedules, exact in-place payload APIs, sequence-bound nonces, a wiped nonce/exporter owner, typed failures, RFC A.2 vectors for all modes, and Native/WASI/Embedded runtime validation; P-256 DHKEM remains separate work |
...
| ECH RFC 9849 | Required | Strict config-list parsing/selection, immutable X25519 key snapshots, HPKE ClientHelloInner/Outer sealing/opening, padding, acceptance confirmation, authenticated reject/retry outcome, and Stream/QUIC core integration are callable; input encapsulation/ciphertext remain borrowed and output is sealed directly into its final owner; DNS, retry transport, and key rotation acquisition are external; HelloRetryRequest continuation remains required |
...
| HPKE X25519 | RFC 9180 Base/PSK/Auth/AuthPSK, all SHA-2 KDF and required AEAD combinations, stack-scoped DH output, prepared AEAD ownership, sequence nonces, exporter, and caller-owned in-place payload operations | RFC A.2 vectors for every mode; all nine KDF/AEAD combinations through two sequential messages; Native/WASI/Embedded execution of all three AEADs through sequence two; 165-test ASan/UBSan passes; committed pre-optimization formal memory and timing artifacts | Refresh formal memory evidence for the prepared-context layout, pass every `1.10x` timing confidence-bound gate, add independent differential interoperability, P-256 DHKEM, automated constant-time review, TSan on a working pinned toolchain, and security review |
...
| Remaining cryptography | Additional approved hybrid constructions and P-256 DHKEM remain outside the callable implementation; ML-DSA-44/65/87, ML-KEM, X25519MLKEM768, and X25519 HPKE are callable; RSA-PSS, RSA PKCS #1 v1.5 SHA-2, and NIST ECDSA are verification-only | Native vectors cover all FIPS 204 ML-DSA parameter sets, FIPS 203, and RFC 9180 X25519 HPKE; ML-DSA and X25519MLKEM768 have production-path and three-target execution; both RSA verification constructions have independent vectors, real X.509 routes, sanitizer evidence where available, and three-target runtime evidence; NIST ECDSA has independent verification vectors | P-256 DHKEM, additional approved hybrid constructions, and their completion gates |
...
| TLS/DTLS | One record-independent TLS 1.3 semantic core shared by Stream and QUIC, narrow client/server protocols, HKDF key schedule, distinct Server-Finished application and Client-Finished resumption-secret owners, AEAD Stream record adapter, synchronous X25519 or X25519MLKEM768 key exchange with Ed25519 authentication for all three TLS 1.3 AEAD suites, paired encrypted NewSessionTicket/server-state issuance, PSK extension/binder primitives, server ticket-age validation, single-use PSK resumption, RFC 9849 ECH accept/reject, post-handshake KeyUpdate, SAN-only identity helper, and bounded DTLS replay window | RFC 8448 handshake/application/exporter/resumption and ticket-PSK known answers; Native classical, Hybrid, ECH, and PSK Stream/Core/QUIC completion; server/client ticket PSK equality; Ed25519 CertificateVerify with X.509 fixtures; failure tests; focused AddressSanitizer; ECH bidirectional pinned-BoringSSL interoperability; replay-window boundaries; and dedicated Native/WASI/Embedded runtime validation | Broader RFC 8448 corpus, 0-RTT, ECH HelloRetryRequest continuation, external credential/trust capabilities, DTLS flights/ACK/timers/epochs, broader allocation/copy measurement, and full-handshake external interoperability |
...
| `SwiftSSL` façade | Explicit AES-GCM, SHA-2, SHA-3/SHAKE, HMAC-SHA-256, HKDF-SHA-256, X25519, Ed25519, ML-KEM-768/1024, ML-DSA-44/65/87, and verification-only P-256 ECDSA adapters; lower modules remain separately importable products | Symmetric, hash/MAC/KDF, X25519, Ed25519, ML-KEM, all ML-DSA parameter-set success/failure paths, and P-256 verification have Native/WASI/Embedded WASI execution | Full application-facing TLS composition and a reviewed stability surface |
