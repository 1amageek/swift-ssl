...
| TLS/DTLS | TLS 1.3 HKDF key schedule, AEAD record protector, synchronous X25519/Ed25519 stream handshake engine for all three TLS 1.3 AEAD suites, post-handshake KeyUpdate, single-use resumption state, SAN-only identity helper, and bounded DTLS replay window | Native handshake completion, tampered-flight rejection, certificate-window rejection, KeyUpdate request/response and post-update data, all suite round trips, resumption known answer, SAN identity tests, replay-window boundaries, and Native/WASI/Embedded WASI target validation | Resumption handshake integration, 0-RTT, external credential/trust capabilities, DTLS flights/ACK/timers/epochs, full state/interop target evidence |
