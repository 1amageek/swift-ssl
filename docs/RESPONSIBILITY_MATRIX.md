...
| Entropy/DRBG | Explicit entropy source and fork/reseed-safe DRBG | Required | Shared DRBG state uses one mutex contract on all targets |
...
| `SwiftSSL` façade | Explicit AES-GCM, SHA-256, HMAC-SHA-256, and HKDF-SHA-256 protocol adapters; lower modules remain separately importable products | AES-GCM façade has Native/WASI/Embedded WASI execution; SHA-256, HMAC-SHA-256, and HKDF-SHA-256 have Native/WASI/Embedded WASI execution | Full application-facing TLS composition and a reviewed stability surface |
