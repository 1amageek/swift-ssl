...
| Remaining cryptography | ML-KEM, ML-DSA, and approved hybrid constructions remain outside the callable implementation; X25519 HPKE is callable; RSA-PSS and NIST ECDSA are verification-only | Native vectors cover RFC 9180 X25519 HPKE A.2 base plus all four modes and all supported KDF/AEAD combinations; RSA-PSS has Montgomery differential, sanitizer, and three-target runtime evidence; NIST ECDSA has independent verification vectors | TLS interoperability, ML-KEM, ML-DSA, ECH, hybrid constructions, and their completion gates |
...
| `SwiftSSL` façade | Explicit AES-GCM, SHA-2, SHA-3/SHAKE, HMAC-SHA-256, HKDF-SHA-256, X25519, Ed25519, and verification-only P-256 ECDSA adapters; lower modules remain separately importable products | Symmetric, hash/MAC/KDF, X25519, Ed25519 success/failure, and P-256 verification have Native/WASI/Embedded WASI execution | Full application-facing TLS composition and a reviewed stability surface |
| SHA-256 benchmark | Separate manual harness and BoringSSL comparison driver | Formal 30-pair Native measurement with confidence intervals is recorded in README; the 1.10x release gate failed for every workload | Optimize 1 KiB/16 KiB throughput and 64-byte confidence bound, then repeat the formal gate; allocation/copy and WASM/Embedded measurements remain open |
