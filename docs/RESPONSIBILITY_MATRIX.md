...
| TLS resumption | Required | Immutable, identity-bound, expiring, single-use state plus PSK ClientHello/binder integration; the state and ticket transport are implemented, while the resumed handshake remains required |
...
| TLS/DTLS | TLS 1.3 HKDF key schedule, AEAD record protector, synchronous X25519/Ed25519 stream handshake engine for all three TLS 1.3 AEAD suites, encrypted NewSessionTicket transport, single-use resumption state, post-handshake KeyUpdate, SAN-only identity helper, and bounded DTLS replay window | Native handshake completion, tampered-flight rejection, certificate-window rejection, encrypted ticket round trip and resumption-state derivation, KeyUpdate request/response and post-update data, all suite round trips, resumption known answer, SAN identity tests, replay-window boundaries, and Native/WASI/Embedded WASI target validation | PSK ClientHello/binder resumption, 0-RTT, external credential/trust capabilities, DTLS flights/ACK/timers/epochs, full state/interop target evidence |
