...
| QUIC RFC 9001 | Required | TLS handshake bytes and secret events only; `QUICInitialSecrets` implements RFC 9001 v1 Initial HKDF derivation, while `QUICCryptoStreamReassembler` owns bounded per-encryption-level ordered delivery |
...
| QUIC TLS integration | Profile actions, checked batch, noncopyable traffic-secret event, ordered action/secret step output, RFC 9001 v1 Initial secret/key/IV/header-protection derivation, and bounded per-encryption-level CRYPTO reassembly with zero-copy contiguous delivery | Focused Native Initial-secret vector, six CRYPTO reassembly tests repeated three times under the hang guard, dedicated Native/WASI/Embedded WASI CRYPTO runtime validation, and existing action/secret tests | TLS handshake-message framing/engine integration, capability correlation, packet protection, allocation/copy measurement, fuzzing, and interoperability |
