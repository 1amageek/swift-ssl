...
| X.509 | Certificate/TBSCertificate/Validity/signature/SPKI structural parser, SPKI, and unencrypted PKCS #8 owners | Native X509 tests for certificate ranges/time fields, X25519 SPKI, and PKCS #8 version failures | Signature validation, extension semantics, path building, identity, revocation, interoperability |
| TLS/DTLS | TLS 1.3 HKDF key schedule, AEAD record protector, and sealed profile/action-batch models | Native SHA-256/SHA-384 schedule derivation tests, all three AEAD suite round trips, padding, sequence and authentication-failure paths | TLS handshake engine, DTLS state, capability suspension, full state/interop/target evidence |
