...
| TLS resumption | Required | Immutable, identity-bound, expiring, single-use state plus PSK ClientHello/binder integration; ticket/state/extension/binder primitives are implemented, while resumed-handshake selection and transcript integration remain required |
...
| TLS/DTLS | TLS 1.3 HKDF key schedule, AEAD record protector, synchronous X25519/Ed25519 stream handshake engine for all three TLS 1.3 AEAD suites, encrypted NewSessionTicket transport, PSK extension/binder primitives, single-use resumption state, post-handshake KeyUpdate, SAN-only identity helper, and bounded DTLS replay window | Native handshake completion, tampered-flight rejection, certificate-window rejection, encrypted ticket round trip and resumption-state derivation, PSK extension round trip and binder verification, KeyUpdate request/response and post-update data, all suite round trips, resumption known answer, SAN identity tests, replay-window boundaries, and Native/WASI/Embedded WASI target validation | Resumed PSK handshake selection/transcript integration, 0-RTT, external credential/trust capabilities, DTLS flights/ACK/timers/epochs, full state/interop target evidence |
...
| `SwiftSSL` façade | Explicit AES-GCM, SHA-2, SHA-3/SHAKE, HMAC-SHA-256, HKDF-SHA-256, and X25519 adapters; lower modules remain separately importable products | Symmetric, SHA-2, SHA-3/SHAKE, HMAC-SHA-256, HKDF-SHA-256, and X25519 have Native/WASI/Embedded WASI execution | Full application-facing TLS composition and a reviewed stability surface |
