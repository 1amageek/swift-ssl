...
enum TargetValidationCommand {
  ...
  private static func validateP256() throws {
    ...
    let alice = try SwiftSSL.P256PrivateKey(bytes: aliceScalar.span)
    let bob = try SwiftSSL.P256PrivateKey(bytes: bobScalar.span)
    let shared = try SwiftSSL.P256.sharedSecret(
      privateKey: alice,
      peerPublicKey: bob.publicKey()
    )
    ...
    privateScalar[31] = 1
    let signingPrivateKey = try SwiftSSL.P256PrivateKey(bytes: privateScalar.span)
    guard try SwiftSSLCrypto.P256ECDSA.verify(
      signature: rawSignature.span,
      messageHash: digest.span,
      publicKey: directSigningKey
    ) else {
    ...
    let signingVectorDigest = ContiguousArray<UInt8>([
      0x17, 0x2B, 0x12, 0x96, 0xFE, 0xDD, 0xD5, 0xE2,
      0xC0, 0xB6, 0x30, 0x06, 0x15, 0x14, 0x2D, 0x3C,
      0x4F, 0x6D, 0x37, 0x5E, 0x5C, 0xB7, 0x0E, 0xD8,
      0xCF, 0xA9, 0x22, 0x0C, 0xCE, 0xB9, 0x4B, 0xE2,
    ])
    let expectedDeterministicSignature = ContiguousArray<UInt8>([
      0x6B, 0x11, 0x71, 0x7B, 0x7E, 0xB4, 0x57, 0x58,
      0x00, 0x10, 0x7E, 0x60, 0xF6, 0x3E, 0x28, 0x4A,
      0x07, 0xD7, 0x85, 0x2E, 0xFC, 0x91, 0x93, 0x63,
      0x53, 0x01, 0xF2, 0x29, 0xBE, 0x6E, 0xEA, 0x80,
      0x0F, 0x94, 0x5E, 0xAC, 0x0C, 0xA9, 0xE8, 0x52,
      0xB6, 0x96, 0x63, 0xB8, 0x2C, 0x33, 0xC8, 0xE1,
      0x95, 0x5D, 0x8C, 0xC6, 0x4A, 0x96, 0x7B, 0x32,
      0xF6, 0x31, 0x81, 0x28, 0xAB, 0x37, 0x64, 0x6A,
    ])
    guard try SwiftSSL.P256ECDSA.sign(
      messageHash: signingVectorDigest.span,
