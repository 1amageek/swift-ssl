...
final class ChaCha20Poly1305Tests: XCTestCase {
    ...
    func testExactInPlaceSealAndOpen() throws {
        ...
        try storage.withUnsafeMutableBufferPointer { buffer in
            var output = MutableSpan(_unsafeElements: buffer)
            try cipher.seal(
                plaintext: Span(_unsafeElements: UnsafeBufferPointer(start: buffer.baseAddress, count: plaintext.count)),
        ...
        try storage.withUnsafeMutableBufferPointer { buffer in
            var output = MutableSpan(_unsafeElements: buffer)
            try cipher.open(
                ciphertextAndTag: Span(_unsafeElements: UnsafeBufferPointer(start: buffer.baseAddress, count: plaintext.count + 16)),
                authenticatedData: Span(_unsafeElements: empty),
                nonce: nonce.span,
                into: &output
            )
    ...
    func testPartialOverlapIsRejectedBeforeMutation() throws {
        ...
        try storage.withUnsafeMutableBufferPointer { buffer in
            ...
            let outputBuffer = UnsafeMutableBufferPointer(start: buffer.baseAddress!.advanced(by: 1), count: 80)
