...
public enum X25519 {
  public static func sharedSecret(
    ...
    let implementation: SwiftSSLCrypto.X25519SharedSecret
    do {
      implementation = try SwiftSSLCrypto.X25519.sharedSecret(
...
public struct X25519PrivateKey: ~Copyable, Sendable {
  public static let byteCount = SwiftSSLCrypto.X25519PrivateKey.byteCount
...
public struct X25519PublicKey: Sendable, Equatable {
  public static let byteCount = SwiftSSLCrypto.X25519PublicKey.byteCount

  fileprivate let implementation: SwiftSSLCrypto.X25519PublicKey

  public init(bytes: Span<UInt8>) throws(CryptoInputError) {
    do {
      implementation = try SwiftSSLCrypto.X25519PublicKey(bytes: bytes)
    } catch {
      throw CryptoInputError(error)
    }
  }

  fileprivate init(implementation: SwiftSSLCrypto.X25519PublicKey) {
    self.implementation = implementation
  }
...
public struct X25519SharedSecret: ~Copyable, Sendable {
  public static let byteCount = SwiftSSLCrypto.X25519SharedSecret.byteCount

  private let implementation: SwiftSSLCrypto.X25519SharedSecret

  fileprivate init(implementation: consuming SwiftSSLCrypto.X25519SharedSecret) {
    self.implementation = implementation
  }
