...
public struct DTLSClientHandshake<C: CryptoProvider>: Sendable {
    ...
    private mutating func handleServerHello(_ body: [UInt8]) throws(DTLSError) {
        ...
        guard sh.extendedMasterSecret, sh.renegotiationInfo else {
            throw DTLSError.handshakeFailed("DTLS WebRTC profile requires EMS and secure renegotiation")
        }
        switch (requiredSRTPProfiles, sh.useSRTP) {
        case (nil, nil):
            selectedSRTPProfile = nil
        case (nil, .some):
            throw DTLSError.handshakeFailed("Server selected SRTP without a client offer")
        case (.some, nil):
            throw DTLSError.handshakeFailed("Server did not negotiate required SRTP")
        case (.some(let offered), .some(let selected)):
            guard selected.protectionProfiles.count == 1,
                  let profile = selected.protectionProfiles.first,
                  offered.contains(profile) else {
                throw DTLSError.handshakeFailed("Server selected an SRTP profile that was not offered")
            }
            guard selected.mki.isEmpty else {
                throw DTLSError.handshakeFailed("Server returned an unexpected SRTP MKI")
            }
    ...
    public mutating func finishClientFlight(
        ...
        let finishedHash = transcriptContext.hash(messages: handshakeMessages, cipherSuite: cipherSuite)
