...
public struct DTLSEngineOutput: Sendable {
    ...
    /// Timer token that atomically reflects the engine state after this receive.
    /// `nextDelay == nil` means no timeout should be scheduled. A final flight may
    /// still be retained internally for duplicate-flight recovery.
    public var retransmissionState: DTLSEngineRetransmissionState

    /// Plaintext record recipes generated by this receive. This remains internal:
    /// only the engine may retain/re-encode handshake material.
    var generatedFlightRecords: [DTLSFlightRecord]
    ...
    public init(
        ...
        anomalies: [Anomaly] = [],
        retransmissionState: DTLSEngineRetransmissionState = .init(
            generation: 0,
            nextDelay: nil
        )
    ) {
