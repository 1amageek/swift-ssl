...
public struct DTLSEngineConfiguration<C>: Sendable where C: Sendable {
    ...
    /// Required SRTP profiles in local preference order, or `nil` for ordinary DTLS.
    /// The engine currently supports empty MKI only.
    public var srtpProtectionProfiles: [SRTPProtectionProfile]?
    ...
    /// Concrete provider operations used by the DTLS 1.2 PRF. This immutable
    /// owner removes associated-type crypto dispatch from the handshake runtime.
    public var prfContext: DTLSPRFContext?

    /// Concrete provider operations used to hash the handshake transcript.
    public var transcriptContext: DTLSTranscriptContext?
    ...
    /// Creates the keyed record-protection owner after the handshake derives a
    /// directional key block. The concrete provider strategy supplies this seam,
    /// keeping associated-type AEAD metadata out of the generic record engine.
    public var recordProtectorFactory: (@Sendable (
        _ cipherSuite: DTLSCipherSuite,
        _ key: [UInt8],
        _ fixedIV: [UInt8]
    ...
    public init(
        ...
        srtpProtectionProfiles: [SRTPProtectionProfile]? = nil,
        ...
        self.srtpProtectionProfiles = srtpProtectionProfiles
        ...
        self.prfContext = nil
        self.transcriptContext = nil
        self.recordProtectorFactory = nil
    }

    package func recordProtector(
        cipherSuite: DTLSCipherSuite,
        key: [UInt8],
        fixedIV: [UInt8]
    ) throws(DTLSEngineError) -> DTLSRecordProtectionContext {
        guard let recordProtectorFactory else {
            throw .invalidConfiguration(reason: "DTLS record protector factory is unavailable")
        }
        do {
            return try recordProtectorFactory(cipherSuite, key, fixedIV)
        } catch {
            throw .internalError(reason: "DTLS record protector construction failed: \(error)")
        }
    }
}
