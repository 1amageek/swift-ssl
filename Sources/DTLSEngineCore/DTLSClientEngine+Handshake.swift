...
extension DTLSClientEngine {
    ...
    private mutating func handleRecord(
        ...
        switch contentType {
        case .handshake:
            try ingestHandshakeRecord(fragment, progress: &progress, into: &output)
            return false
    ...
    private mutating func ingestHandshakeRecord(
        ...
        for fragment in fragments {
            ...
            try dispatchHandshakeMessage(message, progress: &progress, into: &output)
        }
    }
    ...
    private mutating func dispatchHandshakeMessage(
        ...
        // A fully authenticated/reassembled message below the FSM's receive gate is
        // a retransmission of the peer's previous flight. Do not feed it into the
        // transcript; ask the flight controller to resend our current flight after
        // the complete datagram has been processed.
    ...
    mutating func applyActions(
        ...
        for action in actions {
            ...
            case .expectChangeCipherSpec:
                break // Read keys install on receiving CCS (installReadKeys).
