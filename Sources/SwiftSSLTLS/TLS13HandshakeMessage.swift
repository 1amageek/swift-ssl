...
public enum TLS13HandshakeCodec {
  ...
  public static func makeEncryptedExtensions(
    ...
    guard retryByteCount <= UInt16.max - 4 else {
      throw .malformedMessage
    }
    var body = try Self.makeBuilder(
    ...
    do {
      if let echRetryConfigurations {
        try body.appendUInt16BigEndian(UInt16(4 + retryByteCount))
        try body.appendUInt16BigEndian(
          ECHClientHelloCodec.encryptedClientHelloExtensionType
        )
        try body.appendUInt16BigEndian(UInt16(retryByteCount))
  ...
  public static func parseEncryptedExtensions(
    _ message: Span<UInt8>
  ) throws(TLS13HandshakeError) -> ECHConfigList? {
    ...
    do {
      ...
      while !cursor.isAtEnd {
        ...
        guard type == ECHClientHelloCodec.encryptedClientHelloExtensionType else {
  ...
  internal static func truncatedClientHelloForBinder(
    ...
    let preSharedKey = try preSharedKeyForBinderTruncation(message)
  ...
  private static func parseClientExtensions(
    ...
    do {
      ...
      return (serverName, keyShareGroup, keyShare, preSharedKey)
    } catch let error as TLS13HandshakeError {
      throw error
