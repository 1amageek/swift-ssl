...
/// FIPS 203 ML-KEM-1024 exposed through SSL-owned key and result types.
public enum MLKEM1024: InPlaceKeyEncapsulationMechanism {
  ...
  public struct PublicKey: Sendable, Equatable {
    public static let byteCount = SSLCrypto.MLKEM1024.PublicKey.byteCount
    fileprivate let implementation: SSLCrypto.MLKEM1024.PublicKey

    public init(bytes: Span<UInt8>) throws(KEMError) {
      do {
        implementation = try SSLCrypto.MLKEM1024.PublicKey(bytes: bytes)
      } catch {
        throw KEMError(error)
      }
    }

    fileprivate init(implementation: SSLCrypto.MLKEM1024.PublicKey) {
  ...
  public struct PrivateKey: ~Copyable, Sendable {
    public static let byteCount = SSLCrypto.MLKEM1024.PrivateKey.byteCount
    fileprivate let implementation: SSLCrypto.MLKEM1024.PrivateKey

    public init(bytes: Span<UInt8>) throws(KEMError) {
      do {
        implementation = try SSLCrypto.MLKEM1024.PrivateKey(bytes: bytes)
      } catch {
        throw KEMError(error)
      }
    }

    fileprivate init(implementation: consuming SSLCrypto.MLKEM1024.PrivateKey) {
      self.implementation = implementation
    }
  ...
  public struct Encapsulation: Sendable, Equatable {
    public static let byteCount = SSLCrypto.MLKEM1024.Encapsulation.byteCount
    fileprivate let implementation: SSLCrypto.MLKEM1024.Encapsulation

    public init(bytes: Span<UInt8>) throws(KEMError) {
      do {
        implementation = try SSLCrypto.MLKEM1024.Encapsulation(bytes: bytes)
      } catch {
        throw KEMError(error)
      }
    }

    fileprivate init(implementation: SSLCrypto.MLKEM1024.Encapsulation) {
      self.implementation = implementation
    }
  ...
  public struct SharedSecret: ~Copyable, Sendable {
    public static let byteCount = SSLCrypto.MLKEM1024.SharedSecret.byteCount
    fileprivate let implementation: SSLCrypto.MLKEM1024.SharedSecret

    fileprivate init(implementation: consuming SSLCrypto.MLKEM1024.SharedSecret) {
      self.implementation = implementation
    }
  ...
  public static func generateKeyPair(
    ...
    let pair:
      SSLCrypto.KEMKeyPair<
        SSLCrypto.MLKEM1024.PublicKey,
        SSLCrypto.MLKEM1024.PrivateKey
      >
    do {
      pair = try SSLCrypto.MLKEM1024.generateKeyPair(using: entropy)
    } catch {
      throw KEMError(error)
    }
    return KEMKeyPair(
  ...
  public static func generateKeyPair() throws(KEMError) -> KEMKeyPair<PublicKey, PrivateKey> {
    let pair:
      ...
      >
    do {
      pair = try SSLCrypto.MLKEM1024.generateKeyPair()
    } catch {
      throw KEMError(error)
    }
    return KEMKeyPair(
  ...
  public static func encapsulate(
    ...
    let result:
      SSLCrypto.EncapsulationResult<
        SSLCrypto.MLKEM1024.Encapsulation,
        SSLCrypto.MLKEM1024.SharedSecret
      >
    do {
      result = try SSLCrypto.MLKEM1024.encapsulate(
  ...
  public static func encapsulate(
    ...
    let result:
      ...
      >
    do {
      result = try SSLCrypto.MLKEM1024.encapsulate(to: publicKey.implementation)
    } catch {
      throw KEMError(error)
    }
    return EncapsulationResult(
  ...
  public static func decapsulate(
    ...
    do {
      return SharedSecret(
        implementation: try SSLCrypto.MLKEM1024.decapsulate(
