...
public struct SHA3_256Context: ~Copyable, HashContext {
  public static let digestByteCount = SSLCrypto.SHA3_256Context.digestByteCount
  private var implementation: SSLCrypto.SHA3_256Context

  public init() { implementation = SSLCrypto.SHA3_256Context() }

  private init(implementation: consuming SSLCrypto.SHA3_256Context) {
...
public struct SHA3_512Context: ~Copyable, HashContext {
  public static let digestByteCount = SSLCrypto.SHA3_512Context.digestByteCount
  private var implementation: SSLCrypto.SHA3_512Context

  public init() { implementation = SSLCrypto.SHA3_512Context() }

  private init(implementation: consuming SSLCrypto.SHA3_512Context) {
    self.implementation = implementation
  }
...
public enum SHA3_256: HashFunction {
  ...
  public static func hash(_ input: Span<UInt8>, into output: inout MutableSpan<UInt8>)
    throws(CryptoInputError)
  {
    do { try SSLCrypto.SHA3_256.hash(input, into: &output) } catch {
...
public enum SHA3_512: HashFunction {
  ...
  public static func hash(_ input: Span<UInt8>, into output: inout MutableSpan<UInt8>)
    throws(CryptoInputError)
  {
    do { try SSLCrypto.SHA3_512.hash(input, into: &output) } catch {
      throw CryptoInputError(error)
    }
  }
}
