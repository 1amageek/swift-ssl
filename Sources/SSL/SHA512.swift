...
public enum SHA512: HashFunction {
  ...
  {
    do { try SSLCrypto.SHA512.hash(input, into: &output) } catch {
...
public enum SHA384: HashFunction {
  ...
  public static func hash(_ input: Span<UInt8>, into output: inout MutableSpan<UInt8>)
    throws(CryptoInputError)
  {
    do { try SSLCrypto.SHA384.hash(input, into: &output) } catch {
      throw CryptoInputError(error)
    }
  }
}
...
public struct SHA512Context: ~Copyable, HashContext {
  ...
  private var implementation: SSLCrypto.SHA512Context
  public init() { implementation = SSLCrypto.SHA512Context() }
  private init(implementation: consuming SSLCrypto.SHA512Context) {
  ...
  public borrowing func clone() -> SHA512Context {
    SHA512Context(implementation: implementation.clone())
  }
...
public struct SHA384Context: ~Copyable, HashContext {
  ...
  private var implementation: SSLCrypto.SHA384Context
  public init() { implementation = SSLCrypto.SHA384Context() }
  private init(implementation: consuming SSLCrypto.SHA384Context) {
    self.implementation = implementation
  }
  public mutating func update(_ input: Span<UInt8>) throws(CryptoInputError) {
    do { try implementation.update(input) } catch { throw CryptoInputError(error) }
  ...
  public borrowing func clone() -> SHA384Context {
    SHA384Context(implementation: implementation.clone())
  }
  public consuming func finalize(into output: inout MutableSpan<UInt8>) throws(CryptoInputError) {
    do { try implementation.finalize(into: &output) } catch { throw CryptoInputError(error) }
  }
