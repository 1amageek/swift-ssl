...
public enum MLKEM1024: InPlaceEncodedPublicKeyEncapsulationMechanism {
  ...
  public struct PublicKey: Sendable, Equatable {
    ...
    public init(bytes: Span<UInt8>) throws(KEMError) {
      try MLKEMCore.validateEncapsulationKey(bytes, parameters: .mlKEM1024)
      storage = OwnedBytes(copying: bytes)
      expanded = try MLKEMCore.expandEncapsulationKey(bytes, parameters: .mlKEM1024)
    }
  ...
  public struct PrivateKey: ~Copyable, Sendable {
    public static let byteCount = 3_168
    private let storage: SecretBytes
    fileprivate let expanded: MLKEMExpandedPrivateKey

    public init(bytes: Span<UInt8>) throws(KEMError) {
      try MLKEMCore.validateDecapsulationKey(bytes, parameters: .mlKEM1024)
      let publicKeyStart = MLKEMParameters.mlKEM1024.pkePrivateKeyByteCount
      let publicKeyEnd = publicKeyStart + MLKEMParameters.mlKEM1024.encapsulationKeyByteCount
