...
public struct AESGCM: ~Copyable, AuthenticatedCipher {
    ...
    public mutating func seal(
        ...
        guard Self.validMessageLength(plaintext.count), Self.validMessageLength(authenticatedData.count) else {
        ...
        guard !Self.overlaps(plaintext, output, allowSameStart: true),
              !Self.overlaps(authenticatedData, output, allowSameStart: false) else {
    ...
    public mutating func open(
        ...
        guard Self.validMessageLength(ciphertextCount), Self.validMessageLength(authenticatedData.count) else {
            throw .messageLimitReached
        }
    ...
    private func authenticationTag(
        ...
        Self.writePair(hash.0 ^ Self.readUInt64(encryptedCounter, offset: 0), hash.1 ^ Self.readUInt64(encryptedCounter, offset: 8), into: &result)
        return result
    }
    ...
    private static func overlaps(
        ...
        input.withUnsafeBytes { inputBytes in
            output.withUnsafeBytes { outputBytes in
                ...
                let (outputEnd, outputOverflow) = outputStart.addingReportingOverflow(UInt(outputBytes.count))
    ...
    private static func readUInt64<B: Collection>(_ bytes: B, offset: Int) -> UInt64 where B.Element == UInt8 {
        var result: UInt64 = 0
        var index = 0
        while index < 8 {
            result = (result << 8) | UInt64(bytes[bytes.index(bytes.startIndex, offsetBy: offset + index)])
            index += 1
        }
        return result
    }
