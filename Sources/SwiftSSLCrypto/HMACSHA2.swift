    ...
    static func initialize(
        ...
        try keyBlock.withUnsafeMutableBufferPointer { buffer throws(CryptoInputError) -> Void in
            ...
            if key.count > blockByteCount {
                ...
                var index = 0
                while index < key.count { pointer[index] = key[index]; index += 1 }
            }
            var index = 0
            while index < blockByteCount { pointer[index] ^= 0x36; index += 1 }
            ...
            index = 0
            while index < blockByteCount { pointer[index] ^= 0x36 ^ 0x5C; index += 1 }
...
public struct HMACSHA512Context: ~Copyable, MessageAuthenticationCodeContext {
    ...
    public mutating func update(_ input: Span<UInt8>) throws(CryptoInputError) { try state.update(input) }
    public consuming func finalize(into output: inout MutableSpan<UInt8>) throws(CryptoInputError) { try state.finalize(into: &output) }
    public consuming func isValidAuthenticationCode(_ authenticationCode: Span<UInt8>) throws(CryptoInputError) -> Bool { try state.isValid(authenticationCode) }
}
