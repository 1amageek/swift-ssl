...
private enum HKDFSHA2Core<MAC: MessageAuthenticationCode> {
    ...
    static func extract(input: Span<UInt8>, salt: Span<UInt8>, into output: inout MutableSpan<UInt8>) throws(HKDFError) {
        ...
        do { try MAC.authenticate(input, using: salt, into: &output) }
        catch { throw map(error) }
    }

    static func expand(prk: Span<UInt8>, info: Span<UInt8>, into output: inout MutableSpan<UInt8>) throws(HKDFError) {
        ...
        do {
            ...
            if count == digestByteCount {
                ...
                try context.finalize(into: &block)
                var index = 0
                while index < count { output[offset + index] = temporary[index]; index += 1 }
            }
            offset += count
            counter &+= 1
            }
...
public enum HKDFSHA384: ExtractAndExpandKeyDerivationFunction {
    ...
    public static func extract(inputKeyMaterial: Span<UInt8>, salt: Span<UInt8>, into output: inout MutableSpan<UInt8>) throws(HKDFError) { try HKDFSHA2Core<HMACSHA384>.extract(input: inputKeyMaterial, salt: salt, into: &output) }
    public static func expand(pseudorandomKey: Span<UInt8>, info: Span<UInt8>, into output: inout MutableSpan<UInt8>) throws(HKDFError) { try HKDFSHA2Core<HMACSHA384>.expand(prk: pseudorandomKey, info: info, into: &output) }
}
...
public enum HKDFSHA512: ExtractAndExpandKeyDerivationFunction {
    ...
    public static func extract(inputKeyMaterial: Span<UInt8>, salt: Span<UInt8>, into output: inout MutableSpan<UInt8>) throws(HKDFError) { try HKDFSHA2Core<HMACSHA512>.extract(input: inputKeyMaterial, salt: salt, into: &output) }
    public static func expand(pseudorandomKey: Span<UInt8>, info: Span<UInt8>, into output: inout MutableSpan<UInt8>) throws(HKDFError) { try HKDFSHA2Core<HMACSHA512>.expand(prk: pseudorandomKey, info: info, into: &output) }
}
