...
public struct ChaCha20Poly1305: ~Copyable, AuthenticatedCipher {
    ...
    private func validate(
        ...
        guard Self.validMessageLength(messageByteCount),
              Self.validLength(authenticatedDataByteCount) else {
        ...
        input.withUnsafeBytes { inputBytes in
            output.withUnsafeBytes { outputBytes in
                guard inputBytes.count > 0, outputBytes.count > 0,
                      ...
                      let outputBase = outputBytes.baseAddress else { return false }
...
private final class Poly1305 {
    ...
    deinit {
        wipe(&r0); wipe(&r1); wipe(&r2); wipe(&r3); wipe(&r4)
        wipe(&h0); wipe(&h1); wipe(&h2); wipe(&h3); wipe(&h4)
        wipe(&pad0); wipe(&pad1); wipe(&bufferCount)
    ...
    private func addAndMultiply(
        ...
        h0 += m0; h1 += m1; h2 += m2; h3 += m3; h4 += m4 + hibit
        ...
        h0 = d0 & 0x3ffffff
        var value = d1 + carry
        carry = value >> 26; h1 = value & 0x3ffffff
        value = d2 + carry
        carry = value >> 26; h2 = value & 0x3ffffff
        value = d3 + carry
        carry = value >> 26; h3 = value & 0x3ffffff
        value = d4 + carry
        carry = value >> 26; h4 = value & 0x3ffffff
        h0 += carry * 5
        carry = h0 >> 26
        h0 &= 0x3ffffff
        h1 += carry
