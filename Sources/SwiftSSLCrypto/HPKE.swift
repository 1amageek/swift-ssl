...
public struct HPKESenderContext: ~Copyable, Sendable {
    ...
    private func makeNonce() -> ContiguousArray<UInt8> {
        ...
        secretState.withBorrowedBytes { state in
            let bytes = state.extracting(aead.keyByteCount..<(aead.keyByteCount + HPKEAEAD.nonceByteCount))
            var index = 0
            while index < nonce.count { nonce[index] = bytes[index]; index += 1 }
        }
        var index = 0
...
public enum HPKEX25519 {
    ...
    private static func makeRecipient(
        ...
        if let senderPublicKey {
            ...
            do { sender = try X25519PublicKey(bytes: senderPublicKey.span) }
    ...
    private static func finishSender(
        ...
        kemContext.reserveCapacity(encapsulation.count + recipientPublicKey.count + (senderPublicKey?.count ?? 0))
    ...
    private static func append(
        ...
        var index = 0
        while index < source.count { destination.append(source[index]); index += 1 }
    }
...
private enum HPKEPrimitives {
    ...
    static func makeSecret(
        ...
        var bytes = bytes
        defer { wipe(&bytes) }
        do { return try SecretBytes(copying: bytes.span) }
        catch { throw .primitive(.invalidLength(expected: 1, actual: bytes.count)) }
    }
    ...
    private static func labeledExpand(
        ...
        guard length >= 0, length <= 65_535,
              length <= 255 * kdf.digestByteCount else {
