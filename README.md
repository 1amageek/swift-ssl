...
| PKI | X.509 parsing, path construction, policy processing, hostname verification, revocation inputs, trust-provider boundary | Structural X.509 Certificate/TBSCertificate/Validity/signature/SPKI parser is implemented with owned DER ranges; signature validation, extensions, path policy, and trust engine planned |
| TLS | TLS 1.3, resumption, 0-RTT policy, client authentication, key update, ECH, certificate compression, delegated credentials, raw public keys | TLS 1.3 HKDF key schedule and AEAD record seal/open with sequence numbers, TLSInnerPlaintext padding, and typed failure paths are implemented; handshake state engine planned |
