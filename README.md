...
| Public-key crypto | X25519, NIST P-256/P-384/P-521, Ed25519, RSA-PSS, ML-KEM, ML-DSA, approved hybrid groups | RFC 7748 X25519, NIST P-256 ECDH, and P-256 ECDSA verification are present; P-384/P-521, signing, RSA-PSS, and post-quantum algorithms remain required |
...
| PKI | X.509 parsing, path construction, policy processing, hostname verification, revocation inputs, trust-provider boundary | Strict X.509 validity, Ed25519 and P-256 ECDSA-with-SHA2 certificate-signature verification, v3 extension parsing, and SAN-only DNS identity matching are implemented; path construction, policy, revocation, and trust engine remain required |
...
| Public façade | Curated application-facing API without blanket lower-module exports | Explicit AES-GCM, ChaCha20-Poly1305, SHA-256, HMAC-SHA-256, HKDF-SHA-256, X25519, P-256 ECDH, and P-256 ECDSA verification adapters |
...
| P-256 ECDSA verification | Fixed-width `r || s` verifier, strict DER ECDSA signature decoding, SHA-256/384/512 certificate verification, P-256 SPKI validation, and mutation failures | Independent raw-signature vector; real X.509 certificates signed with ECDSA-with-SHA256, SHA384, and SHA512; malformed/modified signature failures | Constant-time audit, independent differential corpus, measured allocation/copy counts, target execution, formal benchmark, and security review |
