...
| TLS | TLS 1.3, resumption, 0-RTT policy, client authentication, key update, ECH, certificate compression, delegated credentials, raw public keys | Record-independent client/server state-machine cores now emit ordered handshake bytes and scoped traffic-secret owners for both full and PSK-resumed X25519 handshakes; the existing stream engine supplies Ed25519 and explicit ECDSA P-256/P-384/P-521 CertificateVerify selection, RFC 5915 EC private-key ingestion, all three TLS 1.3 AEAD suites, KeyUpdate, encrypted NewSessionTicket transport, and the AEAD record layer; the stream adapter still needs to be migrated onto the shared semantic core, while ECDSA authentication remains release-gated pending constant-time, differential, sanitizer, and performance evidence |
