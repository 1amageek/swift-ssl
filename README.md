...
| PKI | X.509 parsing, path construction, policy processing, hostname verification, revocation inputs, trust-provider boundary | SPKI and unencrypted PKCS #8 owners are implemented; certificate parser/validation engine planned |
| TLS | TLS 1.3, resumption, 0-RTT policy, client authentication, key update, ECH, certificate compression, delegated credentials, raw public keys | TLS 1.3 AEAD record seal/open with HKDF-Expand-Label, sequence numbers, TLSInnerPlaintext padding, and typed failure paths are implemented; handshake/key schedule engine planned |
