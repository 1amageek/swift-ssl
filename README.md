...
| QUIC | RFC 9001 handshake adapter and traffic-secret events; QUIC packet protection remains owned by the QUIC stack | Ordered action/secret output, RFC 9001 v1 Initial secret derivation, and bounded per-encryption-level CRYPTO stream reassembly are implemented; TLS engine integration and packet integration remain required |
...
| QUIC CRYPTO reassembly | Per-encryption-level sliding ring, exact retransmission handling, transactional conflicting-overlap rejection, bounded offsets/window, and zero-copy contiguous `Span` delivery including ring wrap | Six focused Native tests with three guarded repetitions and dedicated Native/WASI/Embedded WASI runtime validation | TLS handshake-message framing/engine integration, allocation measurement, fuzzing, interoperability, and release benchmark |
