...
| TLS | TLS 1.3, resumption, 0-RTT policy, client authentication, key update, ECH, certificate compression, delegated credentials, raw public keys | A synchronous X25519 TLS 1.3 client/server handshake engine with Ed25519 and explicit ECDSA P-256/P-384/P-521 CertificateVerify selection, RFC 5915 EC private-key ingestion with derived-public-key matching, all three TLS 1.3 AEAD suites, post-handshake KeyUpdate, encrypted NewSessionTicket transport, PSK extension/binder primitives, server ticket age validation, single-use PSK resumption, HKDF key schedule, and AEAD record layer are implemented; ECDSA authentication remains release-gated pending constant-time, differential, sanitizer, and performance evidence; 0-RTT, ECH, and broader credential policies remain required |
...
| QUIC | RFC 9001 handshake adapter and traffic-secret events; QUIC packet protection remains owned by the QUIC stack | Ordered action/secret output, RFC 9001 v1 Initial secret derivation, bounded per-encryption-level CRYPTO stream reassembly, and zero-copy TLS handshake-message framing are implemented; TLS state-machine integration and packet integration remain required |
...
| ECDSA P-256/P-384/P-521 and RSA-PSS | RFC 6979 P-256/P-384/P-521 raw signing plus fixed-width raw-signature verification, bounded RSA modular exponentiation, strict DER signature decoding including P-521 long-form lengths, SHA-256/384/512 certificate verification, curve/RSA SPKI validation, mutation failures, and TLS 1.3 CertificateVerify | RFC 6979 P-256/P-384/P-521 signing vectors; independent raw-signature vectors; real X.509 certificates signed with ECDSA-with-SHA256, SHA384, SHA512, and RSA-PSS-SHA256; P-256/P-384/P-521 TLS handshake completion; malformed/modified signature failures; targeted ASan passes for P-256/P-384/P-521 signing | Constant-time audit, independent differential corpus, measured allocation/copy counts, target execution, formal benchmark, and security review; TLS ECDSA selection is connected but not release-qualified until these gates pass |
...
| TLS 1.3 records | Three RFC 8446 AEAD suites, TLS 1.3 nonce construction, outer header/AAD, sequence-number limits, inner content-type/padding, authentication-failure no-write behavior, Native record tests, explicit CertificateVerify signature-scheme codec, P-256/P-384/P-521 CertificateVerify handshake paths, and DER long-form length handling | RFC 8448 known-answer record corpus, P-256/P-384/P-521 handshake completion, differential/interop tests, target execution, sanitizer/code-generation review, and release benchmark |
...
| QUIC CRYPTO/TLS stream | Per-encryption-level sliding ring, exact retransmission handling, transactional conflicting-overlap rejection, bounded offsets/window, exact TLS handshake boundaries, and zero-copy message `Span` delivery including ring wrap | Native reassembly/framing/adapter tests, three guarded adapter repetitions, focused AddressSanitizer execution, and dedicated Native/WASI/Embedded WASI runtime validation | TLS state-machine integration, allocation measurement, fuzzing, interoperability, and release benchmark |
