...
| PKI | X.509 parsing, path construction, policy processing, hostname verification, revocation inputs, trust-provider boundary | Strict X.509 validity, Ed25519 certificate-signature verification, and v3 extension parsing are implemented; path construction, policy, identity, revocation, and trust engine remain required |
| TLS | TLS 1.3, resumption, 0-RTT policy, client authentication, key update, ECH, certificate compression, delegated credentials, raw public keys | A synchronous X25519/Ed25519 TLS 1.3 client/server handshake engine, HKDF key schedule, and AEAD record layer are implemented for the pinned AES-128-GCM profile; resumption, 0-RTT, key update, ECH, and broader credential policies remain required |
