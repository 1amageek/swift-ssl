...
| Public-key crypto | X25519, NIST P-256/P-384/P-521, Ed25519, RSA-PSS, ML-KEM, ML-DSA, approved hybrid groups | RFC 7748 X25519, NIST P-256/P-384/P-521 ECDH, ECDSA P-256 signing/verification, ECDSA P-384/P-521 verification, and RSA-PSS SHA-256/384/512 verification are present as validation paths; protocol signing and post-quantum algorithms remain gated or required |
...
| ECDSA P-256/P-384/P-521 and RSA-PSS | RFC 6979 P-256 raw signing plus fixed-width raw-signature verification, bounded RSA modular exponentiation, strict DER signature decoding, SHA-256/384/512 certificate verification, curve/RSA SPKI validation, and mutation failures | RFC 6979 P-256 signing vector; independent raw-signature vectors; real X.509 certificates signed with ECDSA-with-SHA256, SHA384, SHA512, and RSA-PSS-SHA256; malformed/modified signature failures | Constant-time audit, independent differential corpus, measured allocation/copy counts, target execution, formal benchmark, and security review; TLS selection remains rejected until these gates pass |
...
| TLS 1.3 records | Three RFC 8446 AEAD suites, TLS 1.3 nonce construction, outer header/AAD, sequence-number limits, inner content-type/padding, authentication-failure no-write behavior, Native record tests, explicit CertificateVerify signature-scheme codec, and P-256 production-selection rejection test | RFC 8448 known-answer record corpus, differential/interop tests, target execution, sanitizer/code-generation review, and release benchmark |
...
`.test-artifacts/sanitizers/targeted-ecdh2/asan.xcresult`. This does not clear
the full sanitizer release gate.
