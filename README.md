...
| TLS | TLS 1.3, resumption, 0-RTT policy, client authentication, key update, ECH, certificate compression, delegated credentials, raw public keys | A synchronous X25519/Ed25519 TLS 1.3 client/server handshake engine, all three TLS 1.3 AEAD suites, post-handshake KeyUpdate, HKDF key schedule, and AEAD record layer are implemented; resumption, 0-RTT, ECH, and broader credential policies remain required |
